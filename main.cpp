...
//  원본 서열(원본 게놈) 추출 + SNP 심어서 Reference 게놈 생성
//
//  [개념 정리]
//   - original (원본/내 게놈) : 정답이 되는 진짜 서열. read는 여기서 뽑힌다.
//   - reference (레퍼런스 게놈) : original 에 SNP 를 심어 "조금 다르게" 만든 서열.
//                                매칭 알고리즘이 이 reference 를 기준으로 read 를 찾는다.
//   - SNP : 한 글자(염기)가 다른 글자로 바뀐 변이 (substitution)
//
//  [파이프라인]
//   sequence.fasta  --(1단계)-->  original_1M.txt   (원본 100만 염기)
//   original_1M.txt --(2단계)-->  reference_genome.txt (SNP 0.1% 삽입)
//                                  snp_list.txt        (변이 위치 정답표)
//
//  여기까지가 우리 팀 담당. 이후 read 생성/매칭은 다른 팀원이 이어서 진행.
//
...
static const double      MUTATION_RATE = 0.001;   // 돌연변이(SNP) 비율 0.1%
static const unsigned    SEED          = 42;      // 고정 시드 -> 팀원 모두 동일 데이터 생성
// -------------------------------------------------------------

// FASTA 에서 헤더(>로 시작)를 제외하고 ATCG 염기만 모아 N개 추출한다.
// 대문자 변환 + ATCG 이외 문자(혹시 모를 N, 소문자 등)는 건너뛰어 안전하게 처리.
...
// 나머지 3개 중 하나를 균등하게 선택하므로 반드시 다른 글자가 된다.
...
int main() {
    ...
    std::cout << "[2단계] SNP " << (MUTATION_RATE * 100) << "% 삽입하여 reference 게놈 생성 중...\n";
